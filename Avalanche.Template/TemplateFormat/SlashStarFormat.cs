...
/// <summary>String format that uses placeholders as "/*ParameterName*/". </summary>
public class SlashStarFormat : TemplateFormatBase.TextIsBreakdown
{
    /// <summary>Singleton</summary>
    static SlashStarFormat instance = new SlashStarFormat();
    /// <summary>Singleton</summary>
    public static SlashStarFormat Instance => instance;

    /// <summary>'/*'</summary>
    protected static readonly ConstantTokenizer openerTokenizer = new ConstantTokenizer("/*");
    /// <summary>'*/'</summary>
    protected static readonly ConstantTokenizer closerTokenizer = new ConstantTokenizer("*/");
    /// <summary>'#name#'</summary>
    protected static readonly PlaceholderTokenizer placeholderTokenizer = new PlaceholderTokenizer(openerTokenizer, CharTokenizer.AlphaNumeric, IntegerTokenizer.Instance, new UntilTokenizer(closerTokenizer, false, null), closerTokenizer);
    /// <summary>Singleton</summary>
    static TemplateTextTokenizer tokenizer = new TemplateTextTokenizer(new UntilTokenizer(openerTokenizer, false, null), placeholderTokenizer, MalformedTokenizer.Instance);
    /// <summary>Singleton</summary>
    public static TemplateTextTokenizer Tokenizer => tokenizer;

    /// <summary></summary>
    public SlashStarFormat() : base("SlashStar") { }
    ...
    protected override bool TryBreakdown(string text, out ITemplateBreakdown result)
    {
        ...
        if (text == null)
        {
            TemplateBreakdown formatString_ = new TemplateBreakdown { TemplateFormat = this };
        ...
        // Tokenize
        if (!tokenizer.TryTake(text, out CompositeToken tokens)) { result = default!; return false; }
        // Place parts here
        templateBreakdown.Parts = new ITemplatePart[tokens.Children.Length];
        StructList6<ITemplatePlaceholderPart> placeholders = new();
        // Convert tokens to parts
        for (int i = 0; i < tokens.Children.Length; i++)
        {
            // Get token
            IToken token = tokens.Children[i];
            // Place part here
            ITemplatePart part;
            ...
            switch (token)
            {
                case TextToken textToken:
                    part = new TemplateTextPart { TemplateBreakdown = templateBreakdown }.SetTexts(token.Memory).SetReadOnly();
                    break;
                case PlaceholderToken placeholderToken:
                    var parameterPart = placeholderToken.Parameter == null ? null : new TemplateParameterPart { TemplateBreakdown = templateBreakdown }.SetTexts(placeholderToken.Parameter.Memory);
                    var alignmentPart = placeholderToken.Alignment == null ? null : new TemplateAlignmentPart { TemplateBreakdown = templateBreakdown }.SetTexts(placeholderToken.Alignment.Memory).SetReadOnly();
                    var formattingPart = placeholderToken.Formatting == null ? null : new TemplateFormattingPart { TemplateBreakdown = templateBreakdown }.SetTexts(placeholderToken.Formatting.Memory).SetReadOnly();
